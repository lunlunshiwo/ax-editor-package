...
var IconPickerControl = /** @class */ (function (_super) {
    ...
    IconPickerControl.prototype.renderFontIcons = function () {
        var _this = this;
        var _a = this.props, className = _a.className, inputOnly = _a.inputOnly, placeholder = _a.placeholder, cx = _a.classnames, name = _a.name, value = _a.value, noDataTip = _a.noDataTip, formItem = _a.formItem;
        var options = this.formatOptions();
        var vendors = this.getVendors();
        var selectedOptions = formItem && formItem.selectedOptions.length
            ? formItem.selectedOptions
            : value
                ? [{ label: value, value: value }]
                : [];
        return (react_1.default.createElement(downshift_1.default, { isOpen: this.state.isOpen, inputValue: this.state.inputValue, onChange: this.handleChange, onOuterClick: this.handleBlur, onStateChange: this.handleStateChange, selectedItem: selectedOptions.map(function (option) { return option['value']; }) }, function (_a) {
            var getInputProps = _a.getInputProps, getItemProps = _a.getItemProps, isOpen = _a.isOpen, inputValue = _a.inputValue, selectedItem = _a.selectedItem;
            var filteredOptions = inputValue && isOpen
                ? match_sorter_1.default(options, inputValue, { keys: ['label', 'value'] })
                : options;
            filteredOptions = filteredOptions.filter(function (option) { return !~selectedItem.indexOf(option.value); });
            return (react_1.default.createElement("div", { className: cx("IconPickerControl-input IconPickerControl-input--withAC", inputOnly ? className : '', {
                ...
                react_1.default.createElement("div", { className: cx('IconPickerControl-valueWrap') },
                    placeholder &&
                        ...
                        !_this.state.inputValue ? (react_1.default.createElement("div", { className: cx('IconPickerControl-placeholder') }, placeholder)) : null,
                    selectedOptions.map(function (option, index) {
                        return inputValue && isOpen ? null : (react_1.default.createElement("div", { className: cx('IconPickerControl-value'), key: index },
                            react_1.default.createElement("i", { className: cx("" + option.value) }),
                            option.label));
                    }),
                ...
                isOpen ? (react_1.default.createElement("div", { className: cx('IconPickerControl-sugsPanel') },
                    ...
                    filteredOptions.length ? (react_1.default.createElement("div", { className: cx('IconPickerControl-sugs', vendors.length > 1
                        ...
                        react_1.default.createElement("i", { className: cx("" + option.value), title: "" + option.value }))); }))) : (react_1.default.createElement("div", { className: cx(vendors.length > 1
                            ? 'IconPickerControl-multiVendor'
                            : 'IconPickerControl-singleVendor') }, noDataTip)))) : null));
        }));
    };
