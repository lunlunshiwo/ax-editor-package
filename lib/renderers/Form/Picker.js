...
var PickerControl = /** @class */ (function (_super) {
    ...
    PickerControl.prototype.fetchOptions = function () {
        ...
        formItem.loadOptions(source, helper_1.createObject(data, {
            value: value,
            op: 'loadOptions'
        }), {
            autoAppend: true
    ...
    PickerControl.prototype.buildSchema = function (props) {
        return tslib_1.__assign(tslib_1.__assign({}, props.pickerSchema), { type: 'crud', pickerMode: true, syncLocation: false, api: props.source, keepItemSelectionOnPageChange: true, valueField: props.valueField, labelField: props.labelField, checkOnItemClick: true, 
            // 不支持批量操作，会乱套
    ...
    PickerControl.prototype.handleModalConfirm = function (values, action, ctx, components) {
        var idx = findIndex(components, function (item) { return item.props.type === 'crud'; });
        this.handleChange(values[idx].items);
        this.close();
    };
    PickerControl.prototype.handleChange = function (items) {
        var _a = this.props, joinValues = _a.joinValues, valueField = _a.valueField, delimiter = _a.delimiter, extractValue = _a.extractValue, multiple = _a.multiple, options = _a.options, setOptions = _a.setOptions, onChange = _a.onChange, autoFill = _a.autoFill, onBulkChange = _a.onBulkChange;
        var value = items;
        if (joinValues) {
            value = items
    ...
    PickerControl.prototype.renderBody = function () {
        var _a = this.props, render = _a.render, selectedOptions = _a.selectedOptions, options = _a.options, multiple = _a.multiple, valueField = _a.valueField, embed = _a.embed;
    ...
    PickerControl.prototype.render = function () {
        var _a = this.props, className = _a.className, cx = _a.classnames, disabled = _a.disabled, render = _a.render, modalMode = _a.modalMode, source = _a.source, size = _a.size, env = _a.env, clearable = _a.clearable, multiple = _a.multiple, placeholder = _a.placeholder, embed = _a.embed, value = _a.value, selectedOptions = _a.selectedOptions;
        return (react_1.default.createElement("div", { className: cx("PickerControl", className) }, embed ? (react_1.default.createElement("div", { className: cx('Picker') }, this.renderBody())) : (react_1.default.createElement("div", { className: cx("Picker", {
            ...
            react_1.default.createElement("div", { onClick: this.handleClick, className: cx('Picker-input') },
                !selectedOptions.length && placeholder ? (react_1.default.createElement("div", { className: cx('Picker-placeholder') }, placeholder)) : null,
