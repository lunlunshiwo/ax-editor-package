...
exports.iRendererStore = mobx_state_tree_1.types
    ...
    parentId: mobx_state_tree_1.types.optional(mobx_state_tree_1.types.string, ''),
    ...
    return {
        ...
        updateData: function (data, tag) {
            if (data === void 0) { data = {}; }
            var prev = self.data;
            var newData;
            if (tag) {
                var proto = helper_1.createObject(self.data.__super || null, tag);
                newData = helper_1.createObject(proto, tslib_1.__assign(tslib_1.__assign({}, self.data), data));
            }
            else {
                newData = helper_1.extendObject(self.data, data);
            }
