...
exports.Row = mobx_state_tree_1.types
    ...
    get expanded() {
        return mobx_state_tree_1.getParent(self, self.depth * 2).isExpanded(self);
    },
    get moved() {
        return self.index !== self.newIndex;
    },
    get locals() {
        return helper_1.createObject(helper_1.createObject(mobx_state_tree_1.getParent(self, self.depth * 2).data, {
            index: self.index
        }), self.data);
    },
    get checkable() {
    ...
    change: function (values, savePristine) {
        self.data = tslib_1.__assign(tslib_1.__assign({}, self.data), values);
        savePristine &&
            (self.pristine = tslib_1.__assign({}, self.data));
    },
    reset: function () {
...
exports.TableStore = iRenderer_1.iRendererStore
    ...
    function getFootableColumns() {
        return self.columns.filter(function (item) {
            return item.type === '__checkme' ||
                ...
                ? false
                : (item.toggled || !item.toggable) &&
                    (self.footable && item.breakpoint && helper_1.isBreakpoint(item.breakpoint));
        });
    }
    function getLeftFixedColumns() {
        ...
        return getFilteredColumns().filter(function (item) { return item.fixed === 'left'; });
    }
    ...
    function combineCell(arr, keys) {
        ...
        for (var i = 1, len = arr.length; i < len; i++) {
            var current = arr[i];
            if (tpl_builtin_1.resolveVariable(key, current.data) == value) {
                row.rowSpans[key] += 1;
